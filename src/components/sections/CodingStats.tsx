...
export const CodingStats = () => {
  ...
  const fetchLC = async (h: string) => {
    ...
    try {
      ...
      const res = await fetch(
        `https://alfa-leetcode-api.onrender.com/${encodeURIComponent(h)}`
      );
      const json = await res.json();
      ...
      setLcData({
        totalSolved:
          json.totalSolved ??
          json.submitStats?.acSubmissionNum?.reduce?.(
            (a: number, c: any) => a + (c.count || 0),
            0
          ),
        easySolved:
          json.easySolved ??
          json.submitStats?.acSubmissionNum?.find?.(
  ...
  const fetchAll = () => {
    if (cfHandle) fetchCF(cfHandle);
    if (lcHandle) fetchLC(lcHandle);
    if (ccHandle) fetchCC(ccHandle);
    if (gfgHandle) fetchGFG(gfgHandle);
  };
      ...
      <div className="container">
        <header className="mb-10 text-center">
          ...
          <p className="text-muted-foreground mt-2">
            Live profiles across platforms
          </p>
        </header>
                ...
                {cfData && (
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <div className="font-semibold">
                        {cfData.rank ?? "—"}
                        {cfData.maxRank ? ` (max: ${cfData.maxRank})` : ""}
                      </div>
                    </div>
                  </div>
                )}
              ...
              <CardContent>
                ...
                {lcData ? (
                  ...
                  <a
                    ...
                    href={`https://leetcode.com/${encodeURIComponent(lcHandle || "")}/`}
                    target="_blank"
                    rel="noreferrer"
                  >
                    Open profile
              ...
              <CardContent>
                ...
                {ccData ? (
                  <div>
                    ...
                    <div className="font-semibold">
                      {ccData.rating ?? "—"}{" "}
                      {ccData.stars ? `(${ccData.stars})` : ""}
                    </div>
                  </div>
                ) : (
                  <a
                    className="story-link"
                    href={`https://www.codechef.com/users/${encodeURIComponent(ccHandle || "")}`}
                    target="_blank"
                    rel="noreferrer"
                  >
                    Open profile
            ...
            <Card className="glass h-full">
              ...
              <CardContent>
                ...
                {gfgData ? (
                  ...
                  <a
                    className="story-link"
                    href={`https://auth.geeksforgeeks.org/user/${encodeURIComponent(gfgHandle || "")}/`}
                    target="_blank"
                    rel="noreferrer"
                  >
                    Open profile
